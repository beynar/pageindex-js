...
import { processMarkdown, type MarkdownProcessingOptions } from '../processing/markdown.js'
...
export class TreeBuilder {
  ...
  private extractPagesFromMarkdownTree(
    tree: TreeNode[],
    _content: string
  ): Array<{ index: number; text: string; tokenCount: number; nodeId?: string }> {
    const pages: Array<{ index: number; text: string; tokenCount: number; nodeId?: string }> = []
   ...
   */
  private async buildFromPdf(document: DocumentInput): Promise<TreeBuildResult> {
    const pdfData =
