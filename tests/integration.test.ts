...
describe("PageIndex with SQLite Storage (no LLM)", () => {
	...
	test("content storage modes", async () => {
		// Test inline mode
		const inlineStorage = createSQLiteStorage(":memory:");
		inlineStorage.initialize();
		try {
		...
		// Test separate mode
		const separateStorage = createSQLiteStorage(":memory:");
		separateStorage.initialize();
		try {
...
describeCerebras("PageIndex with Real LLM (Cerebras)", () => {
	...
	let model: Awaited<
		ReturnType<(typeof import("@ai-sdk/cerebras"))["cerebras"]>
	>;
	...
	test("searches document with LLM reasoning", async () => {
		...
		console.log(
			"Search results:",
			results.map((r) => ({ title: r.node.title, score: r.score })),
		);
	...
	test("retrieves content with context", async () => {
		...
		expect(context.toLowerCase()).toMatch(
			/database_url|api_key|config|variable/i,
		);
	...
	test("uses expert knowledge in search", async () => {
		...
		expect(titles.some((t) => t.includes("two") || t.includes("section"))).toBe(
			true,
		);
